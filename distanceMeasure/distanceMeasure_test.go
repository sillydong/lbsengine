...
func TestCompare(t *testing.T) {
	...
	if err != nil {
		t.Log("打开distance.log失败")
		return
	}
	log.SetOutput(logFile)
	for i := 0; i < TEST_DATA_NUM/100; i++ {
		log.Printf("第%d组数据: EarthPointPair = %v 和 %v, stardard = %f, quick = %f, quick2 = %f\n",
			i, arrPoint1[i], arrPoint2[i], resultStardard[i], resultQuick[i], resultQuick[i])
	}
	...
	t.Log(variance, variance2)
	t.Logf("精度为%f度时，换算以米为单位时表示搜索范围距离在%f米内时:\n标准差组1为%f米,组2为%f米\n",
